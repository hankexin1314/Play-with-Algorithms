...
- **class文件常量池**：`.class`文件中包括：版本，接口，方法还有常量池等信息，存放：字面量（代码中双引号括起来的字符串）和符号引用。

> 字面量：文本字符串，被声明为final的常量

- **运行时常量池**：`.class`文件加载到内存中后，就会将**class文件常量池**复制到**运行时常量池**，每个类都有一个运行时常量池。方法区的一部分。
- 常量池中只有**引用**，真正的字符串在**堆**上
...
- String是final的，在加载阶段，str1的`abc`放入堆，引用放入**常量池**
- 解析str3的时候，会去**常量池**中查找，找到了，所以 ==
- 对str2，会创建两个实例：一个是**字面量**存放在堆中的，引用存放在**常量池**的实例；另一个是存放在堆中的，new出来的**实例**，str2是后者的指针
- str4使用intern，返回**池**中的`def`的引用，如果没有，则将引用添加进去，因为有所以直接返回引用，但是这个引用和str2的引用是不一样的，属于前者
- str5直接从**池**中拿出引用
...
![字符串拼接-常量池2](E:\workspace\Play-with-Algorithms\img\字符串拼接-常量池2.png)

### 3. Java程序运行流程大致介绍

- 代码 `.java`文件，先经过编译器**编译**，变成`.class`文件，也就是**java字节码**
- 然后JVM**加载**硬盘中的字节码执行

# 内存管理

- 内存管理包括**内存区域**，**内存分配**和**垃圾回收**
...
- **虚拟机栈**：栈中的元素是**栈帧**，一个方法的调用对应着栈帧从压入到弹出。每个栈帧中，维持着这个方法的**局部变量表**，动态链接、方法出口信息（可能是地址或句柄等）等信息。

> **局部变量表**存放了：编译期可知的数据类型（基本数据类型），对象引用等
>
> 栈有容量上限，如果不允许自动扩展：**StackOverFlowError**
>
> 如果可动态扩展，扩展到内存耗尽：**OutOfMemoryError**
>
> 栈帧弹出的情况：**return**或者**抛出异常**
...
>
> JDK7及以前，堆主要分为新生代，老年代，永久代

- **方法区**：存放**已经加载的类的信息**，**常量**，**静态变量**、**即时编译的代码**等数据。JDK8以前，方法区就是永久代（不进行垃圾回收），很容易内存溢出，因为方法区有大小上限。JDK8，废弃永久代概念，将**常量**（全局字符串常量池）和**静态变量**放入堆，将其他信息放入**元空间**（直接内存）

> 之前方法区，永久代有一个JVM本身设置的**固定上限**，无法更改，很容易溢出，改为**元空间**，使用直接内存，溢出的概率降低

- **运行时常量池**：属于方法区，存有类版本，字段，方法，接口等信息，还有常量池表（存放编译时生成的字面量和符号引用）。class文件常量池会在类加载后，复制到运行时常量池中
...
> 避免因分配担保机制带来的复制降低效率
>
> 先在Eden，GC后发现Survivor存不下，分配担保，放入老年代

- **长期存活的对象进入老年代**，设置计数器记录年龄，设置threshold定义年龄阈值，默认是15
...
- 类静态属性引用的对象，局部变量表中引用的对象等都可作为`GC roots`

#### 2.3 方法区的回收

- 主要回收目标是**常量池**和**类的卸载**，效率不高
- 为了避免溢出，大量使用反射和动态代理的场景需要回收
- **类卸载**需要满足：所有实例都回收，对应ClassLoader回收，对应的Class对象没有在任何地方被引用，也无法通过反射访问到。
...
- **虚引用**：不会对关联对象生存时间产生影响，无法通过虚引用得到一个对象，关联对象被回收时会收到一个系统通知。用于**跟踪对象的垃圾回收活动**
...
- 跨代引用假说：跨代引用相对于同代引用占极少数（新生代对象被老年代引用，所以每次GC需要扫描老年代）

> 如何解决跨代引用问题

- 在**新生代**上构建一个**记忆集**，将**老年代**分为多块，记录哪一块会存在跨代引用
- Minor GC时，只会额外扫描包含跨代引用的小块内存里的对象，不用对整个老年代扫描

#### 4.2 新生代的回收

- Eden空间满就Minor GC

#### 4.3 老年代的回收（Full GC何时触发）

- **调用System.gc()** ，建议JVM垃圾回收，不一定真回收。不建议使用，建议交给JVM管理
...
- 低停顿意味着更小的新生代空间，更频繁地GC，更低的吞吐量
- **自适应调节策略**，收集器自动根据系统状态调节新生代大小，各区比例，晋升老年代的Threshold来优化。

#### 5.4 Serial Old 收集器

- serial收集器的老年代版本，单线程，标记-整理

#### 5.5 Parallel Old 收集器

- Parallel Scavenge 收集器的老年代版本， 标记-整理
...
- 吞吐量低
- 标记-清除算法导致内存碎片，虽有空间，但是都太小，空间担保失败，提前触发Full GC

- 无法清理**浮动垃圾**（并发清除时产生的垃圾，只能到下一次GC回收），可能出现Concurrent Mode Failure（不能等到老年代快满才进行GC，必须预留空间给浮动垃圾，预留空间不足就会出现这个错误）
...
- **空间整合**：整体看是**标记-整理**算法，对每个Region是**复制**算法（将存活对象复制，然后清除整个Region）
...
- 每个Region都有**记忆集**(Remember Set)，用来记录该Region引用了谁和被谁引用。
- 避免了可达性分析时全盘扫描
...
- **筛选回收**：并发，停顿，首先维护一个**优先级列表**，优先级高低的依据是回收的性价比，然后根据用户所**期望的停顿时间**（可手动设置）来平衡**吞吐量**和**延迟**
...
- 避免类的**重复加载**
- 带有优先级的类加载器关系，使得**基础类的类加载器得到统一**（优先使用启动类加载器加载`lib`文件下的类）
...
- 自定义类加载器，继承`ClassLoader`，重写`findClass`方法即可
...
- 如果要破坏双亲委派，需要重写`loadClass`

# 调优

- 大多数Java应用不需要GC优化，多数导致GC问题的Java应用，都是代码问题
- 减少创建对象的数量；减少适用全局变量和大对象；GC调优是最后不得已的手段
...
- 因为**Full GC**成本远高于**Minor GC**
- 根据GC日志分析，适当增加新生代大小 `-Xmn`，最大限度降低新对象直接进入老年代的情况

> 新生代空间不足会将新对象分配到老年代

### 2. 大对象进入老年代

- 如果大对象进入新生代，很可能造成新生代空间不足，大量年龄不够的**小对象**进入老年代，出现频繁的**Full GC**

> 因为动态的对象年龄判断，新生代中对象过少，会出问题

- 使用`-XX:PretenureSizeThreshold`设置直接进入老年代的对象的大小
