...
- **全局字符串常量池**：**准备**阶段之后，将**堆**中的字符串实例的**引用**存放到池中
...
- String是final的，在加载阶段，str1的`abc`放入堆，引用放入**全局字符串常量池**
- 解析str3的时候，会去**全局字符串常量池**中查找，找到了，所以 ==
- 对str2，会创建两个实例：一个是**字面量**存放在堆中的，引用存放在**全局字符串常量池**的实例；另一个是存放在堆中的，new出来的**实例**，str2是后者的指针
...
- **方法区**：存放**已经加载的类的信息**，**常量**，**静态变量**等数据。JDK8以前，方法区就是永久代（不进行垃圾回收），很容易内存溢出，因为方法区有大小上限。JDK8，废弃永久代概念，将**常量**（全局字符串常量池）和**静态变量**放入堆，将其他信息放入**元空间**（直接内存）

> 之前方法区受限于JVM的内存区域大小，改为元空间，使用直接内存，溢出的概率降低
