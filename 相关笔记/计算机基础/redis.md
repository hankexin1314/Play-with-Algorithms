...
- 很好扩展，应对大数据场景也能应付。
- 但使用分布式缓存需要引入**额外的服务**，比如Redis或者Memcached。而且需要进行远程请求，性能差一些。
...
- **C语言**开发的NoSQL(**非关系型**)数据库

- 与传统数据库不同，Redis的数据存在**内存**中
...
- Memcached支持**多线程**，Redis只支持**单线程**。数据量足够大时Memcached性能优于Redis
...
- 实现ZSet的数据结构，要求：支持随机插入删除，排序，元素唯一

### 2.1 为什么用跳跃表

- 支持随机插入删除：排除数组
- 支持高并发：树形结构的rebalance涉及整棵树的操作，如果数据量大，性能差：排除平衡树，红黑树。而且树形结构**实现起来复杂**。

### 2.2 什么是跳跃表

- 以往查找，使用二分查找，但是链表很难二分查找，只能遍历
- 现在是相邻节点之间有指针，想到每隔一个节点一个指针

![640](E:\workspace\Play-with-Algorithms\img\640.png)

- 依此类推，第三层，每隔两个一个指针...，但是插入节点需要变动的指针太多了

- 插入节点**随机层数**，每次插入只涉及几个指针的**部分变动**

> 默认最大层数是32层

![640](E:\workspace\Play-with-Algorithms\img\640.jpg)
...
- **优点**：对redis性能影响小，因为fork函数。数据恢复时比AOF快（直接复制，相比于AOF重放操作）。
