...
- 遍历所有可能时，要关注是将背包放在外层遍历还是将物品放在外层遍历
...
状态： 第几天，是否持有股票（0， 1），可交易次数（如果为无限次，则不作为状态，因为无法穷举，也没必要）
...
字符串问题一般是两个指针，dp的含义为`word1[0: i] -> word2[0: j]`所需要的最少步骤数 

根据关系得出 插入 删除 替换 对应的状态转化公式，选其中最小的
